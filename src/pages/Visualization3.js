...
const Visualization3 = () => {
  ...
  const beats = new Set(data.beats.map((beat) => Math.ceil(beat.start*1000/100)*100));
  ...
  const renderRectangles = () => {
    const array = [];
    for (let i = 0; i < 100; i++ ) {
      array.push(<rect key={i} className="rectangle" x={i} height={(Math.sin(i) + 2) * 25} width={0.5} style={{fill: "#0000FF"}} />);
    }
    return array;
  }
